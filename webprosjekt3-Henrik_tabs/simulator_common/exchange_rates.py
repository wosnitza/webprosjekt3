...
def load_rates() -> 'DataFrame, Array, Dictionary':
    ...
    try:
        ...
        note = str(len(rates_from_files)) + \
            ' historic exchange rates loaded from available .csv files...'
    except:
...
def add_limits(limit_key: str) -> 'AddToDict':
    ...
    correlation = float(
        correlations[sc.CROSS_BASE_LEG + limit_key[:3] + '-' + sc.CROSS_BASE_LEG + limit_key[3:6]])
    l1_up = float(limits[sc.CROSS_BASE_LEG +
                         limit_key[:3] + limit_key[6:] + '-upper'])
    l2_up = float(limits[sc.CROSS_BASE_LEG +
                         limit_key[3:6] + limit_key[6:] + '-upper'])
    l1_low = float(limits[sc.CROSS_BASE_LEG +
                          limit_key[:3] + limit_key[6:] + '-lower'])
    l2_low = float(limits[sc.CROSS_BASE_LEG +
                          limit_key[3:6] + limit_key[6:] + '-lower'])

    # Apply the formula to the variables
    la_up = math.sqrt(l1_up ** 2 + l2_up ** 2 -
                      (2 * l1_up * l2_up * correlation))
    la_low = math.sqrt(l1_low ** 2 + l2_low ** 2 -
                       (2 * l1_low * l2_low * correlation)) * -1
...
def retrieve_limit(foreign_currency: str, accounting_currency: str, risk_window: int) -> float:
    if foreign_currency == accounting_currency:
        return 1
    limit_key = foreign_currency + accounting_currency + \
        str(risk_window).zfill(3) + '-100'
