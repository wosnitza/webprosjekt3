...
def normalise_terms(start_date: 'date', end_date: 'date', as_string=False) -> int:
    if as_string:
...
def load_from_clipboard_or_file(date_columns: 'List', amount_columns: 'List', required_columns: 'List',
    ...
    for amount_column in amount_columns:
        try:
            input_table[amount_column] = input_table[amount_column].apply(lambda x: gf.convert_excel_amount(x, 1.0))
        except:
            continue
