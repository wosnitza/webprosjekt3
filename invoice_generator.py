...
def generate_list_of_invoices(year: int, amount_of_invoices: int, total_cost: float,
    ...
    payment_terms_distribution = [600, 880, 610, 1445, 137, 95, 23, 18, 7, 1, 1]
    payment_terms = []
    for i in range(len(payment_terms_options)):
        payment_terms.extend(payment_terms_options[i] for x in range(payment_terms_distribution[i]))
    ...
    for currency in currency_distribution.keys():
        ...
        factor_dict[currency + '_tot'] = (total_cost * (currency_distribution[currency]) / 100) / factor_dict[currency]
    ...
    amount_conv = [amount[i] * er.exchange_rate(sc.ACCOUNTING_CURRENCY + currencies[i], v) for i, v in enumerate(due_date)]
